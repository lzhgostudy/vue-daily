...
现在，你已经知道了 `$emit` 的用法，它可以被 `v-on` 侦听，但是Vue实例同时在其事件接口中提供了其他方法。我们可以：
...
- 它需要在这个组件实例中保存这个 `picker` , 如果可以的话最好只有生命周期钩子可以访问到它。这并不是最严重的问题，但是它可以被视为杂物。

- 我们的建立代码独立于我们的清理代码，这使得我们比较难于程序化地清理我们建立的所有东西。

你应该通过一个程序化的侦听器解决这个问题：
...
使用这个策略，我甚至可以让多个输入框元素同时使用不同的Pikaday, 每个新的实例都会程序化地在后期清理它自己：
...
> 注意Vue的事件系统不同于浏览器的EventTarget API。尽管它们工作起来是相似的，但是 `$emit, $on, $off` 并不是 `dispatchEvent, addEventListener, removeEventListener` 的别名。
...
假设你需要构建一个文件目录树，像访达或资源管理器那样的。你可能有一个 `<tree-floder>`组件，模板是这样的：
...
然而，如果你使用一个模板系统依赖/导入组件，例如通过webpack或Browserify，你会遇到一个错误：

```
Failed to mount component: template or render function not defined.
```

为了解释这里发生什么事，我们先把两个组件称为A和B。模块系统发现它需要 A，但是首先 A 依赖 B，但是 B 又依赖 A，但是 A 又依赖 B，如此往复。这变成了一个循环，不知道如何不经过其中一个组件而完全解析出另一个组件。为了解决这个问题，我们需要给模块系统一个点，在那里“A 反正是需要 B 的，但是我们不需要先解析 B。”
...
当 `inline-template` 这个特殊的attribute出现在一个子组件上时，这个组件将会使用其里面的内容作为模板，而不是将其作为被分发的内容。这使得模板的撰写工作更加灵活。
...
感谢Vue的响应式系统，他始终知道何时进行更新（如果你用对了的话）。不过还是有一些边界情况，你想要强制更新，尽管表面上看响应式的数据没有发生改变。也有一些情况是你想阻止不必要的更新。
